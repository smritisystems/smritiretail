    ...
    async def resolve_item_by_barcode_or_sku(
        ...
        if bc_match and bc_match.item:
            ...
            return ItemResolutionResponse(
                ...
                tax_rate=float(variant.tax_rate if (variant and variant.tax_rate is not None) else (item.tax_rate or 0.00)),
                mrp=float(variant.mrp if (variant and variant.mrp is not None) else (item.mrp or 0.00)),
                selling_price=float(variant.selling_price if (variant and variant.selling_price is not None) else (item.selling_price or 0.00)),
                cost_price=float(variant.cost_price if (variant and variant.cost_price is not None) else (item.cost_price or 0.00)),
        ...
        if var_match and var_match.item:
            ...
            return ItemResolutionResponse(
                ...
                barcode=primary_bc,
                tax_rate=float(var_match.tax_rate if var_match.tax_rate is not None else (item.tax_rate or 0.00)),
                mrp=float(var_match.mrp or item.mrp or 0.00),
                selling_price=float(var_match.selling_price or item.selling_price or 0.00),
                cost_price=float(var_match.cost_price or item.cost_price or 0.00),
                primary_uom=item.primary_uom,
                category=item.category,
                brand=item.brand,
            )
        ...
        if item_match:
            ...
            return ItemResolutionResponse(
                ...
                barcode=primary_bc,
                tax_rate=float(item_match.tax_rate or 0.00),
                mrp=float(item_match.mrp or 0.00),
                selling_price=float(item_match.selling_price or 0.00),
