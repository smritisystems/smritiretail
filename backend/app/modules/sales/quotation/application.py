...
class QuotationApplicationService:
    ...
    async def approve_quotation(self, q_id: str) -> SalesQuotation:
        return await self._update_status(q_id, "Approved", allowed_from={"Submitted"})
    ...
    async def convert_to_sales_order(self, q_id: str):
        raise HTTPException(status_code=501, detail="Quotation conversion to Sales Order is not implemented yet")
