...
async def test_master_release_manifest_verification():
    """Verify MasterReleaseManifest contains certified version telemetry for v30.0.0."""
    manifest = MasterReleaseManifest.get_manifest()
    assert manifest["master_version"] == "30.0.0"
    ...
    # Verify domain releases registered (including Phase 36)
    assert len(manifest["domain_releases"]) >= 12
    assert "Phase 24" in manifest["domain_releases"]
    assert "Phase 36" in manifest["domain_releases"]
...
async def test_master_health_probes_pass():
    ...
    assert res["subsystems_checked"] == 18
    assert res["passed_subsystems"] == 18
