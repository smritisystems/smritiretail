...
async def test_concurrent_alias_registration_same_entity(session_factory):
    ...
    """
    alias_code = f"TEST-CONC-{IdentityEngine.generate_technical_id()[:8]}"
    entity_type = "ITEM"
    ...
    async def worker():
        async with session_factory() as session:
            alias = await IdentityEngine.register_alias(
                ...
                alias_type="BARCODE",
                source_system="EXTERNAL_POS",
                company_id=company_id,
            )
            await session.commit()
            return alias.id

    results = await asyncio.gather(*[worker() for _ in range(10)], return_exceptions=True)
    
    # Check that no exceptions were raised
    for r in results:
        assert not isinstance(r, Exception), f"Concurrent registration raised exception: {r}"
    
    # Check that all returned the exact same alias id
    first_id = results[0]
    assert all(r == first_id for r in results), f"Expected identical alias ID across workers, got: {set(results)}"
...
async def test_alias_collision_protection(session_factory):
    ...
    """
    alias_code = f"TEST-COLL-{IdentityEngine.generate_technical_id()[:8]}"
    entity_type = "PARTY"
...
async def test_alias_case_insensitive_lookup_and_registration(session_factory):
    ...
    """
    raw_code = f"EWB-UpPer-{IdentityEngine.generate_technical_id()[:6]}"
