...
Classification: Internal — Database Seeder (TDB-v2.0)
"""
...
def seed_control_capabilities(database_name: str = "smritisys") -> None:
    """
    Seeds platform_capabilities, feature_flags, and module_states into smritisys Control Plane.
    """
    print(f"\n--- Seeding Capability Master Data into Control Plane [{database_name}] ---")
    try:
        conn = psycopg2.connect(f"postgresql://postgres:postgres@localhost:5432/{database_name}")
        cur = conn.cursor()
        ...
        conn.commit()
        conn.close()
        print(f"Successfully seeded control plane data in [{database_name}].")
    except Exception as e:
        print(f"Error seeding [{database_name}]: {e}")


@seed_contract(target="tenant")
def seed_tenant_capabilities(database_name: str) -> None:
    """
    Seeds tenant_capability_bindings and feature_flags into a registered TENANT database.
    """
    print(f"\n--- Seeding Tenant Capabilities into [{database_name}] ---")
    try:
        conn = psycopg2.connect(f"postgresql://postgres:postgres@localhost:5432/{database_name}")
        cur = conn.cursor()
        ...
        conn.commit()
        conn.close()
        print(f"Successfully seeded tenant bindings into [{database_name}].")
    except Exception as e:
        print(f"Error seeding [{database_name}]: {e}")
...
def seed_capability_master_data():
    ...
    """
    seed_control_capabilities("smritisys")
    for db_name in ["smriti001", "smriti002"]:
        seed_tenant_capabilities(db_name)
