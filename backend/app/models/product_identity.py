...
from sqlalchemy import Column, String, Integer, Boolean, Text, ForeignKey, DateTime, UniqueConstraint, text
...
class BarcodeProvider(BaseEntity):
    ...
    config = Column(JSONB, server_default=text("'{}'"), default=dict)
...
class IdentityRule(BaseEntity):
    ...
    scope = Column(JSONB, server_default=text("'{}'"), default=dict)
...
class ProductIdentity(BaseEntity):
    ...
    identity_metadata = Column(JSONB, server_default=text("'{}'"), default=dict)
