...
async def main():
    ...
    async with async_session() as session:
        ...
        prod = Product(id='prod-test', code='PROD-TEST', name='Proto', price=10, stock=10, category='General', barcode='TEST-0001', company_id='comp-test', branch_id='br-test')
        session.add(prod)
        await session.commit()
    ...
    async def reserve(res_id):
        async with async_session() as session:
            ...
            try:
                # instrument internal state without patching service
    ...
    async with async_session() as session:
        reserved = await session.execute(text("SELECT reserved_stock, stock FROM products WHERE id='prod-test'"))
        r = reserved.first()
        count = await session.execute(text("SELECT COUNT(*) FROM stock_movements WHERE product_id='prod-test'"))
        print('final reserved_stock:', r[0], 'final stock:', r[1])
