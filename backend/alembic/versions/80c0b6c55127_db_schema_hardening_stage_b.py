...
def upgrade() -> None:
    ...
    if connection and not context.is_offline_mode() and hasattr(connection, 'execute'):
        try:
            ...
            if null_inv and null_inv > 0 or null_ret and null_ret > 0:
            ...
            """))
            dup_count = result.scalar() if result else 0
    ...
    if connection and not context.is_offline_mode() and hasattr(connection, 'execute'):
        try:
            ...
            for row in products_res:
                ...
                for sec_bc in sec_barcodes:
                    if sec_bc and sec_bc != prim_barcode:
                        sec_bc_id = f"BC-{uuid_pkg.uuid4().hex[:8]}"
                        connection.execute(sa.text("""
                            INSERT INTO product_barcodes (id, uuid, tenant_id, company_id, branch_id, product_id, barcode, is_primary, is_active, is_deleted, version)
