...
export interface SSPAICommand {
  type: "increase_mrp" | "fill_hsn" | "highlight_duplicates" | "enforce_margin" | "format_casing" | "normalize_brands";
  parameter?: number | string;
}

export interface SSPAIResult {
...
/**
 * Parses natural language AI prompts for SSP Spreadsheet Platform.
 */
export function parseSSPAIPrompt(prompt: string): SSPAICommand | null {
  const clean = prompt.trim().toLowerCase();

  const increaseMrpMatch = clean.match(/(?:increase|raise|up|adjust)\s*(?:all\s*)?mrp\s*by\s*(\d+(?:\.\d+)?)\s*%/);
  if (increaseMrpMatch) {
    return { type: "increase_mrp", parameter: parseFloat(increaseMrpMatch[1]) };
  }

  if (clean.includes("hsn")) {
    const hsnMatch = clean.match(/\b\d{4,8}\b/);
    return { type: "fill_hsn", parameter: hsnMatch ? hsnMatch[0] : "61091000" };
  }

  if (clean.includes("duplicate") || clean.includes("dup")) {
    return { type: "highlight_duplicates" };
  }

  if (clean.includes("margin")) {
    const marginMatch = clean.match(/(\d+(?:\.\d+)?)\s*%/);
    return { type: "enforce_margin", parameter: marginMatch ? parseFloat(marginMatch[1]) : 25 };
  }

  if (clean.includes("uppercase") || clean.includes("capitalize")) {
    return { type: "format_casing" };
  }

  if (clean.includes("brand")) {
    return { type: "normalize_brands" };
  }
...
/**
 * Executes AI commands on SSP grid row datasets.
 */
export function executeSSPAICommand(
  rows: Record<string, any>[],
  command: SSPAICommand
): { updatedRows: Record<string, any>[]; result: SSPAIResult } {
    ...
    case "normalize_brands": {
      updatedRows.forEach((r, idx) => {
        if (!r.brand || r.brand.trim() === "") {
          ...
          r.brand = r.brand.trim().toUpperCase();
        }
      });
      summary = `Normalized brand names across ${updatedRowsCount} rows.`;
      break;
    }
  }
