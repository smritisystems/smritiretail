...
const VendorPrintModalBase: React.FC<VendorPrintModalProps> = ({
  ...
  const [tenantName, setTenantName] = useState<string>("Tattly Threads Pvt Ltd");
  ...
  const printableDocument = (
    <div className="vendor-printable-sheet bg-white text-slate-900 p-8 max-w-[820px] mx-auto min-h-[1120px] text-[11px] font-sans leading-relaxed shadow-sm">
      ...
      <div className="text-center text-[8.5px] text-slate-500 mt-6 pt-2 border-t border-slate-200">
        Generated by SMRITI Retail OS • Enterprise Universal Party Master • System-of-Record Guarantee (PostgreSQL + FastAPI) • Page 1 of 1
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-slate-950/80 backdrop-blur-md no-print">
        {/* Top Floating Command Bar */}
        <div className="h-16 px-6 bg-slate-900 border-b border-slate-800 flex items-center justify-between shadow-lg shrink-0">
          ...
          {/* Mode Switcher Pills */}
          <div className="flex items-center bg-slate-800/80 p-1 rounded-xl border border-slate-700/80 space-x-1">
            <button
          ...
          <div className="flex items-center space-x-3">
            <label className="flex items-center space-x-1.5 text-xs text-slate-400 hover:text-slate-200 cursor-pointer">
              <input
                type="checkbox"
            ...
            <button
              onClick={handlePrint}
              className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-lg shadow-indigo-600/30 flex items-center space-x-2 transition"
            >
              <Printer size={15} />
