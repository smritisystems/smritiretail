...
const CATEGORY_BADGES: Record<ContactCategory, { bg: string; text: string; icon: React.ReactNode }> = {
  SALES:      { bg: "bg-blue-500/10 border-blue-500/30",    text: "text-blue-400",    icon: <Briefcase size={12} /> },
  ACCOUNTS:   { bg: "bg-emerald-500/10 border-emerald-500/30", text: "text-emerald-400", icon: <CreditCard size={12} /> },
  LOGISTICS:  { bg: "bg-amber-500/10 border-amber-500/30",  text: "text-amber-400",  icon: <Truck size={12} /> },
  MANAGEMENT: { bg: "bg-purple-500/10 border-purple-500/30", text: "text-purple-400", icon: <Shield size={12} /> },
  GENERAL:    { bg: "bg-slate-500/10 border-slate-500/30",  text: "text-slate-400",  icon: <Users size={12} /> },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-white">First-Class Contact Directory</h3>
          <p className="text-xs text-slate-400">Target PO dispatches, payment advice, and delivery coordination to the right personnel</p>
        </div>
        {isEditing && (
          <button
          ...
          return (
            <div
              ...
              className={`p-4 rounded-xl border transition ${
                c.isPrimary ? "bg-slate-900/90 border-indigo-500/40 shadow-sm" : "bg-slate-900/50 border-slate-800"
              }`}
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="font-bold text-white text-sm flex items-center space-x-1.5">
                    ...
                    {c.isPrimary && (
                      <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center space-x-0.5">
                        <Star size={9} className="fill-emerald-400" />
                        <span>Primary</span>
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">{c.designation || "Representative"}</div>
                </div>
              ...
              {isEditing && !c.isPrimary && (
                <button
                  ...
                  className="mt-3 text-[11px] text-indigo-400 hover:text-indigo-300 font-medium block"
                >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-5 space-y-4 shadow-2xl">
            <h4 className="text-base font-bold text-white">Add Contact Person</h4>
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1">Operational Category</label>
                <select
