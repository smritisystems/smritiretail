  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-white">Registered Offices, Warehouses & Godown Coordinates</h3>
          <p className="text-xs text-slate-400">Dispatch locations for purchase orders, e-Way bills, and multi-state GSTIN deliveries</p>
        </div>
        {isEditing && (
          <button
        ...
        {vendor.addresses.map((addr, idx) => (
          <div
            ...
            className={`p-4 rounded-xl border transition ${
              addr.isPrimary 
                ? "bg-slate-900/90 border-indigo-500/40 shadow-sm shadow-indigo-500/5" 
                : "bg-slate-900/50 border-slate-800"
            }`}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center space-x-2">
                <span className="font-bold text-white text-sm">{addr.addressTitle || "Address"}</span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-mono border border-slate-700">
                  {addr.addressType}
                </span>
                {addr.isPrimary && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 flex items-center space-x-1 font-bold">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-5 space-y-4 shadow-2xl">
            <h4 className="text-base font-bold text-white">Add Delivery / Warehouse Location</h4>
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 mb-1">Location Title</label>
                <input
                  type="text"
