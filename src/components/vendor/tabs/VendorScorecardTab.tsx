...
const SLA_STYLES: Record<SupplierSLAStatus, { bg: string; border: string; text: string; label: string }> = {
  GREEN:    { bg: "bg-emerald-950/20", border: "border-emerald-600/30", text: "text-emerald-400", label: "GREEN — High SLA Compliance" },
  AMBER:    { bg: "bg-amber-950/20",   border: "border-amber-600/30",   text: "text-amber-400",   label: "AMBER — Minor Delays Observed" },
  RED:      { bg: "bg-rose-950/20",    border: "border-rose-600/30",    text: "text-rose-400",    label: "RED — High Rejection / Delay" },
...
export const VendorScorecardTab: React.FC<VendorScorecardTabProps> = ({ vendor }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-bold text-white">Vendor SLA Compliance & Quality Scorecard</h3>
        <p className="text-xs text-slate-400">On-time delivery performance, fill rate percentages, and quality rejection metrics</p>
      </div>

      {/* SLA Badge Banner */}
      <div className={`p-4 rounded-xl border ${style.bg} ${style.border} flex items-center justify-between`}>
        <div>
          <div className="text-xs text-slate-400 font-medium">Composite SLA Health Status</div>
          <div className={`text-lg font-black mt-0.5 ${style.text}`}>{style.label}</div>
        </div>
        <div className="text-right">
          <div className="text-xs text-slate-400">Scorecard Rating</div>
          <div className="text-2xl font-black text-white font-mono">{scorecard?.scorecard || 92} / 100</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
          <div className="text-[11px] font-bold text-slate-400 uppercase">On-Time Delivery</div>
          <div className="text-xl font-black text-emerald-400 font-mono mt-1">
            {scorecard?.onTimeDeliveryPct.toFixed(1) || 95.0}%
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">Contracted Lead Time: 7 Days</div>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
          <div className="text-[11px] font-bold text-slate-400 uppercase">Order Fill Rate</div>
          <div className="text-xl font-black text-indigo-400 font-mono mt-1">
            {scorecard?.fillRatePct.toFixed(1) || 98.3}%
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">Target: 95.0%</div>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
          <div className="text-[11px] font-bold text-slate-400 uppercase">Quality Rejection</div>
          <div className="text-xl font-black text-amber-400 font-mono mt-1">
            {scorecard?.qualityRejectionPct.toFixed(2) || 0.58}%
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">Defect / RMA Rate</div>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
          <div className="text-[11px] font-bold text-slate-400 uppercase">Accrued Delay Penalties</div>
          <div className="text-xl font-black text-slate-300 font-mono mt-1">
            ₹{(scorecard?.totalPenaltyAccrued || 500).toLocaleString("en-IN")}
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">₹500 / Overdue Day</div>
        </div>
      </div>
    </div>
  );
