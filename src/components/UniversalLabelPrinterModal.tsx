...
export const UniversalLabelPrinterModal: React.FC<UniversalLabelPrinterModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150 font-mono text-xs select-none">
      <div className="bg-theme-surface-1 border border-indigo-500/40 w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
        
        {/* Header Bar */}
        <div className="px-6 py-4 bg-theme-surface-2 border-b border-indigo-500/20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-indigo-600/20 border border-indigo-500/40 flex items-center justify-center text-indigo-400">
              <Printer size={20} />
            </div>
            <div>
              <h3 className="text-base font-bold text-theme-heading flex items-center gap-2">
              ...
              <p className="text-[11px] text-theme-muted">Print barcodes & PRN tags directly across all modules without opening individual transactions.</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl bg-theme-surface-3 text-theme-muted hover:text-theme-heading hover:bg-theme-surface-hover transition border border-theme-divider">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-theme-surface-2 p-4 rounded-2xl border border-theme-divider space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input
                    ...
                    className="w-full bg-theme-surface-3 border border-theme-divider rounded-xl pl-9 pr-3 py-1.5 text-xs text-theme-heading outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <select
                  ...
                  className="bg-theme-surface-3 border border-theme-divider text-theme-heading text-xs rounded-xl px-3 py-1.5 outline-none focus:border-indigo-500 font-bold"
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition ${
                    showFilterDrawer ? "bg-indigo-600 border-indigo-400 text-white" : "bg-theme-surface-3 border-theme-divider text-theme-body hover:text-theme-heading"
                  }`}
                >
              ...
              {showFilterDrawer && (
                <div className="pt-3 border-t border-theme-divider grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px] animate-in fade-in duration-150">
                  <div>
                    <label className="text-theme-muted block mb-1">Category</label>
                    <select
                      value={categoryFilter}
                      onChange={e => setCategoryFilter(e.target.value)}
                      className="w-full bg-theme-surface-3 border border-theme-divider text-theme-heading rounded-lg px-2 py-1 outline-none"
                    >
                  ...
                  <div>
                    <label className="text-theme-muted block mb-1">Brand</label>
                    <select
                      value={brandFilter}
                  ...
                  <div>
                    <label className="text-theme-muted block mb-1">Warehouse</label>
                    <select
                  ...
                  <div>
                    <label className="text-theme-muted block mb-1">Stock Status</label>
                    <select
              ...
              {selectionMode === "range_boundary" && (
                <div className="pt-3 border-t border-theme-divider flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="bg-theme-surface-3 border border-theme-divider rounded-lg px-3 py-1 text-xs text-theme-heading outline-none"
                  />
            ...
            <div className="bg-theme-surface-2 border border-theme-divider rounded-2xl overflow-hidden shadow-lg">
              <div className="px-4 py-2.5 bg-theme-surface-3 border-b border-theme-divider flex items-center justify-between text-xs">
                <span className="font-bold text-theme-heading">Records ({targetItemsToPrint.length} Items Selected)</span>
                <span className="text-emerald-400 font-bold">Total Label Output: {totalLabelsToPrint} Copies</span>
              </div>

              <div className="max-h-80 overflow-y-auto">
                <table className="w-full text-left text-xs font-mono">
                  <thead className="bg-theme-surface-2 text-[10px] uppercase text-theme-muted font-bold sticky top-0 border-b border-theme-divider">
                    <tr>
                      <th className="px-3 py-2 text-center">
                        <input
                          ...
                          className="rounded bg-theme-surface-3 border-theme-divider"
                        />
                      </th>
                  ...
                  <tbody className="divide-y divide-theme-divider">
                    {filteredItems.map(item => {
                      ...
                      return (
                        <tr key={item.id} className={`hover:bg-theme-surface-3 ${isChecked ? "bg-indigo-950/20" : "opacity-50"}`}>
                          ...
                          <td className="px-3 py-2 text-theme-heading truncate max-w-xs">{item.name}</td>
                          <td className="px-3 py-2 text-right">
                            <strong className="text-emerald-400">₹{item.price || 0}</strong> / <span className="text-theme-muted">₹{item.mrp || item.price || 0}</span>
                          </td>
                          <td className="px-3 py-2 text-right text-theme-body">{item.stock_qty || 0}</td>
                          <td className="px-3 py-2 text-right">
                            <input
                              type="number"
                              min="1"
                              ...
                              className="w-16 text-right bg-theme-surface-3 border border-theme-divider rounded px-2 py-1 text-theme-heading font-bold outline-none focus:border-indigo-500"
                            />
                          </td>
                        </tr>
                      );
          ...
          <div className="space-y-4">
            ...
            <div className="bg-theme-surface-2 p-4 rounded-2xl border border-theme-divider space-y-3">
              ...
              <select
                ...
                className="w-full bg-theme-surface-3 border border-theme-divider text-theme-heading rounded-xl px-3 py-2 text-xs font-mono outline-none focus:border-indigo-500"
              >
                {templates.map(t => (
              ...
              {showPrnCodeInspector && (
                <div className="space-y-2 pt-2 animate-in fade-in duration-150">
                  <textarea
                    ...
                    rows={6}
                    className="w-full bg-theme-surface-3 border border-theme-divider rounded-xl p-2.5 text-[10px] font-mono text-emerald-400 outline-none"
                  />
                  {isEditingPrnScript && (
                    <button onClick={() => setIsEditingPrnScript(false)} className="text-[10px] text-theme-muted hover:underline">Reset to Default Script</button>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="bg-theme-surface-2 p-4 rounded-2xl border border-theme-divider space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <button
                  ...
                  className={`p-2 rounded-xl border text-left font-bold transition ${quantitySource === "fixed" ? "bg-indigo-600 border-indigo-400 text-white" : "bg-theme-surface-3 border-theme-divider text-theme-muted"}`}
                >
                  Fixed Copies
                </button>
                <button
                  onClick={() => setQuantitySource("stock")}
                  className={`p-2 rounded-xl border text-left font-bold transition ${quantitySource === "stock" ? "bg-indigo-600 border-indigo-400 text-white" : "bg-theme-surface-3 border-theme-divider text-theme-muted"}`}
                >
                  Stock On-Hand
                </button>
                <button
                  onClick={() => setQuantitySource("received")}
                  className={`p-2 rounded-xl border text-left font-bold transition ${quantitySource === "received" ? "bg-indigo-600 border-indigo-400 text-white" : "bg-theme-surface-3 border-theme-divider text-theme-muted"}`}
                >
                  Received (GRN/PO)
                </button>
                <button
                  onClick={() => setQuantitySource("sold")}
                  className={`p-2 rounded-xl border text-left font-bold transition ${quantitySource === "sold" ? "bg-indigo-600 border-indigo-400 text-white" : "bg-theme-surface-3 border-theme-divider text-theme-muted"}`}
                >
                  Sold (POS Sales)
                </button>
              </div>
              ...
              {quantitySource === "fixed" && (
                <div className="flex items-center gap-2 pt-1">
                  <span className="text-theme-muted text-[11px]">Copies per Item:</span>
                  <input
                    type="number"
                    min="1"
                    value={fixedCopies}
                    onChange={e => setFixedCopies(parseInt(e.target.value) || 1)}
                    className="w-20 bg-theme-surface-3 border border-theme-divider rounded-lg px-2.5 py-1 text-theme-heading font-bold outline-none text-right"
                  />
                </div>
              )}
            </div>
            ...
            <div className="bg-theme-surface-2 p-4 rounded-2xl border border-theme-divider space-y-2">
              <span className="text-[10px] text-theme-muted uppercase font-bold block flex items-center justify-between">
                <span>Live Sample Tag Preview</span>
                <Eye size={12} className="text-indigo-400" />
              </span>
              
              <div className="bg-theme-surface-3 p-4 rounded-xl border border-theme-divider flex items-center justify-center min-h-[140px]">
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 bg-theme-surface-2 border-t border-indigo-500/20 flex items-center justify-between">
          <div className="text-xs text-theme-muted">
            Dispatches PRN payload to <strong className="text-indigo-300">{activePrinter.name}</strong>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={onClose} className="px-4 py-2 bg-theme-surface-3 text-theme-body text-xs rounded-xl hover:bg-theme-surface-hover transition border border-theme-divider">
              Cancel
            </button>
            <button
