  ...
  return (
    <div className="bg-[#141726] border border-slate-800 rounded-2xl p-4 space-y-3 shadow-xl font-mono text-xs">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs font-bold">
        <label className="flex items-center gap-2 cursor-pointer p-1.5 rounded bg-[#0a0c14] border border-slate-800 text-emerald-300">
          <input type="checkbox" checked={options.doPrint} onChange={() => toggle("doPrint")} className="accent-emerald-500" />
          <span>☑ Direct Print</span>
        </label>

        <label className="flex items-center gap-2 cursor-pointer p-1.5 rounded bg-[#0a0c14] border border-slate-800 text-indigo-300">
          <input type="checkbox" checked={options.doPreview} onChange={() => toggle("doPreview")} className="accent-indigo-500" />
          <span>☑ Tag Preview</span>
        </label>

        <label className="flex items-center gap-2 cursor-pointer p-1.5 rounded bg-[#0a0c14] border border-slate-800 text-amber-300">
          <input type="checkbox" checked={options.doExportPDF} onChange={() => toggle("doExportPDF")} className="accent-amber-500" />
          <span>☑ Export PDF</span>
        </label>

        <label className="flex items-center gap-2 cursor-pointer p-1.5 rounded bg-[#0a0c14] border border-slate-800 text-slate-300">
