...
export const ScanPrintPanel: React.FC<ScanPrintPanelProps> = ({
  ...
  return (
    <div className="bg-theme-surface-1 border border-theme-divider rounded-2xl p-5 space-y-4 shadow-xl font-mono text-xs max-w-3xl mx-auto select-none">
      ...
      <form onSubmit={handleScanSubmit} className="space-y-2">
        <span className="text-[10px] text-theme-muted uppercase font-bold block">Scan Barcode Code / Stock No Symbol:</span>
        <div className="flex gap-2">
          <input 
            ...
            className="w-full bg-theme-surface-2 border-2 border-emerald-500/50 rounded-xl px-3 py-2 text-emerald-300 font-bold text-sm outline-none shadow-inner" 
          />
      ...
      {/* Status Message */}
      <div className={`p-2.5 rounded-xl border flex items-center gap-2 font-bold text-xs ${isError ? "bg-red-950/60 border-red-500/50 text-red-300" : "bg-theme-surface-2 border-theme-divider text-amber-300"}`}>
        ...
        <div className="bg-theme-surface-2 border border-theme-divider rounded-xl p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2 text-xs">
              <div><span className="text-[9px] text-theme-muted block font-bold uppercase">Item Name:</span><span className="text-theme-heading font-bold text-sm">{scannedItem.name}</span></div>
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-[9px] text-theme-muted block font-bold uppercase">Barcode:</span><span className="text-indigo-300 font-bold">{scannedItem.barcode}</span></div>
                <div><span className="text-[9px] text-theme-muted block font-bold uppercase">Price:</span><span className="text-emerald-400 font-bold">₹{scannedItem.price}</span></div>
              </div>
            </div>

            <div className="bg-theme-surface-1 border border-theme-divider rounded-xl p-2 flex items-center justify-center">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-theme-divider">
            <div className="flex items-center gap-2">
              <span className="text-theme-muted font-bold">Enter Print Copies:</span>
              <input type="number" min={1} max={100} value={qty} onChange={e => setQty(Math.max(1, parseInt(e.target.value) || 1))} className="bg-theme-surface-1 border border-theme-divider rounded px-3 py-1 text-amber-300 font-bold w-20 text-center text-sm" />
            </div>
