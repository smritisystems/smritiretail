      ...
      {activePanel && (
        <>
          ...
          <motion.div 
            ...
            className="fixed right-0 top-0 bottom-0 w-[420px] bg-theme-surface-2 border-l border-theme-divider shadow-2xl z-50 flex flex-col font-sans text-theme-heading"
          >
            {/* Header */}
            <div className="flex justify-between items-center px-5 py-4 border-b border-theme-divider bg-theme-surface-3">
              <div>
                <h2 className="text-lg font-bold font-display text-white">{activePanel.title}</h2>
                <div className="text-xs text-theme-muted uppercase font-mono tracking-wider mt-1 flex items-center space-x-2">
                  <span className="bg-theme-surface-2 text-theme-body px-1.5 py-0.5 rounded text-[10px] font-bold">{activePanel.entityType}</span>
                  <span>•</span>
                  <span>{activePanel.entityId}</span>
                </div>
              </div>
              <button onClick={closePanel} className="text-theme-muted hover:text-rose-400 transition-colors p-1 rounded-lg hover:bg-white/5">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            
            {/* Contextual Toolbar */}
            <div className="flex bg-theme-surface-3 border-b border-theme-divider p-2 space-x-1.5 shrink-0">
               {activePanel.entityType === "customer" && customer ? (
                 <>
                   ...
                   <button 
                     ...
                     className="flex-1 flex justify-center items-center space-x-1 py-1.5 bg-theme-surface-2 hover:bg-theme-surface-hover border border-theme-divider rounded text-xs font-semibold text-white transition-colors"
                   >
                 ...
                 <>
                   <button className="flex-1 flex justify-center items-center space-x-1 py-1.5 bg-theme-surface-2 hover:bg-theme-surface-hover border border-theme-divider rounded text-xs font-semibold text-white transition-colors">
                     <span className="material-symbols-outlined text-[14px]">edit</span>
                     <span>Edit</span>
                   </button>
                   <button onClick={(e) => { e.stopPropagation(); window.print(); }} className="flex-1 flex justify-center items-center space-x-1 py-1.5 bg-theme-surface-2 hover:bg-theme-surface-hover border border-theme-divider rounded text-xs font-semibold text-white transition-colors">
                     <span className="material-symbols-outlined text-[14px]">print</span>
                     <span>Print</span>
                   </button>
                 </>
            ...
            {/* Tab Selector */}
            {activePanel.entityType === "customer" && customer && (
              <div className="flex border-b border-theme-divider bg-theme-surface-3 px-4 shrink-0">
                <button
               ...
               {activePanel.entityType === 'customer' && customer && group && policy ? (
                 activeTab === "profile" ? (
                   ...
                   <div className="space-y-2">
                     ...
                     <div className="bg-theme-surface-3 border border-theme-divider rounded-xl p-4 space-y-3">
                       <div className="flex justify-between border-b border-theme-divider pb-2">
                     ...
                     <div className="bg-theme-surface-3 border border-theme-divider rounded-xl p-4 space-y-3">
                       ...
                       <div className="flex items-center space-x-2 pt-2 border-t border-theme-divider">
                         <input
                           ...
                           id="add-tag-input"
                           className="flex-1 bg-theme-surface-2 border border-theme-divider rounded-lg px-2.5 py-1 text-xs text-white focus:outline-none focus:border-indigo-500/80 font-mono"
                           onKeyDown={(e) => {
                             if (e.key === "Enter") {
                   ...
                   <div className="space-y-2">
                     ...
                     <div className="bg-theme-surface-3 border border-theme-divider rounded-xl p-4 flex items-center justify-between">
                             ...
                             className={`px-2.5 py-1 text-xs font-bold rounded transition-colors ${
                               customer.status === st
                                 ? st === "Active"
                                   ...
                                   : "bg-theme-surface-3 text-white"
                                 : "bg-theme-surface-2 text-theme-muted hover:bg-theme-surface-hover hover:text-white"
                             }`}
                           >
                             {st}
                           </button>
                   ...
                   <div className="space-y-2">
                     ...
                     <div className="bg-theme-surface-3 border border-theme-divider rounded-xl p-4 space-y-4">
                       <div className="grid grid-cols-2 gap-4">
                         <div className="bg-theme-surface-2 border border-theme-divider p-2.5 rounded-lg text-center">
                       ...
                       {!isUnlimited && (
                         <div className="space-y-1.5">
                           ...
                           <div className="w-full bg-theme-surface-2 rounded-full h-2 overflow-hidden">
                             <div 
                   ...
                   <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-theme-surface-3 border border-theme-divider rounded-xl overflow-hidden divide-y divide-theme-divider font-sans">
                        {getSalesInvoices().filter(inv => inv.customerId === customer.id).map(inv => (
                          <div key={inv.id} className="p-3 flex justify-between items-center hover:bg-theme-surface-2/30 transition-colors">
                            <div>
                        ...
                        {getSalesReturns().filter(ret => {
                          ...
                          <div key={ret.id} className="p-3 flex justify-between items-center hover:bg-theme-surface-2/30 transition-colors">
                            <div>
                     ...
                     <div className="relative pl-6 border-l border-theme-divider space-y-6">
                       {timelineActivities.map((act) => (
                         <div key={`${act.type}-${act.id}`} className="relative">
                           {/* Left bullet marker */}
                           <span className={`absolute -left-[31px] top-1.5 w-4 h-4 rounded-full border border-theme-divider flex items-center justify-center shadow-lg ${
                             act.type === "Quotation" ? "bg-blue-500" :
                             act.type === "Order" ? "bg-amber-500" :
                             "bg-emerald-500"
                           }`}>
                             <span className="material-symbols-outlined text-[10px] text-theme-heading font-bold">
                               {act.icon}
                             </span>
                           </span>

                           {/* Timeline Card */}
                           <div className="bg-theme-surface-3 border border-theme-divider hover:border-theme-divider transition-colors p-3.5 rounded-xl space-y-2.5">
                 ...
                 <>
                   ...
                   <div>
                      ...
                      <div className="space-y-2">
                         <button className="w-full text-left bg-theme-surface-3 hover:bg-theme-surface-2 border border-theme-divider rounded-lg p-3 transition-colors group flex justify-between items-center">
                            <div>
                            ...
                            <span className="material-symbols-outlined text-theme-muted group-hover:text-blue-400">arrow_forward_ios</span>
                         </button>
                         <button className="w-full text-left bg-theme-surface-3 hover:bg-theme-surface-2 border border-theme-divider rounded-lg p-3 transition-colors group flex justify-between items-center">
                            <div>
