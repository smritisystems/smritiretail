...
const _TrendCardComponent: React.FC<TrendCardProps> = ({
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 48 }}>
        {data.map((d, i) => (
          <div key={i} title={`${d.label}: ${d.value}${unit ?? ""}`} style={{
            ...
            background: i === data.length - 1 ? "var(--c-seef-accent)" : "rgba(129,140,248,0.35)",
      ...
      {/* Last value */}
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--c-theme-muted)" }}>
        <span>{data[0]?.label}</span>
        <span style={{ fontWeight: 600, color: "var(--c-theme-body)" }}>
