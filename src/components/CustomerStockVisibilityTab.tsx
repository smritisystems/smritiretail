 ...
 * Module       : Customer Stock Visibility & Modern Trade Studio (ADR-CSV-001)
...
import {
  ...
  Filter, Calendar, Clock, DollarSign, Sparkles
} from "lucide-react";
import { CustomerStockVisibilityEngine, CustomerStoreStockRecord } from "../services/customerStockVisibilityEngine.ts";
...
export const CustomerStockVisibilityTab: React.FC = () => {
  ...
  const filteredRecords = useMemo(() => {
    return stockRecords.filter((r) => {
      const matchCust = selectedCustomerFilter === "ALL" || r.customerName === selectedCustomerFilter;
      const matchSearch =
        searchQuery === "" ||
  ...
  }, [stockRecords, selectedCustomerFilter, searchQuery]);

  const handleSimulateSellOutImport = () => {
    setIsImporting(true);
    setTimeout(() => {
      const res = engine.importSellOutData([
        { customerCode: "CUST-001", storeCode: "WH-01", sku: "SHOE-001", sellOutQty: 10, sellOutDate: new Date().toISOString().slice(0, 10) },
        { customerCode: "CUST-002", storeCode: "WH-02", sku: "TSHIRT-001", sellOutQty: 15, sellOutDate: new Date().toISOString().slice(0, 10) },
      ]);
    ...
    <div className="flex flex-col h-full bg-theme-base text-theme-body font-sans">
      ...
      <div className="p-5 border-b border-theme-divider bg-theme-surface-1 flex justify-between items-center">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold font-display text-theme-heading">Customer Stock Visibility (Modern Trade &amp; Off-Balance Stock)</h1>
          </div>
          <p className="text-xs text-theme-muted mt-0.5">
            Commercial visibility of inventory lying at customer stores (Reliance, D-Mart, Lifestyle, Croma) post GST Tax Invoice
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Upload size={14} />
            <span>{isImporting ? "Importing Sell-Out..." : "Import Sell-Out CSV / EDI"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-5 grid grid-cols-1 md:grid-cols-4 gap-4 bg-theme-surface-2 border-b border-theme-divider">
        <div className="p-4 bg-theme-surface-1 border border-theme-divider rounded-xl">
          <span className="text-[10px] text-theme-muted font-bold uppercase block">Invoiced to Customers</span>
          <div className="text-xl font-black text-white mt-1">{totalInvoiced.toLocaleString()} Units</div>
          <span className="text-[10px] text-theme-muted">Cumulative Tax Invoiced</span>
        </div>
      ...
      {/* Filter Bar */}
      <div className="px-5 py-3 border-b border-theme-divider bg-theme-surface-1 flex flex-wrap justify-between items-center gap-3 font-mono text-xs">
          ...
          <table className="w-full text-left border-collapse text-xs font-mono">
            <thead>
              <tr className="bg-theme-surface-2 border-b border-theme-divider text-[10px] font-bold uppercase text-theme-muted">
                <th className="p-3">Customer Account / Store Location</th>
                ...
                <th className="p-3">Inventory Aging (0-30 / 31-60 / 61+ Days)</th>
            ...
            <tbody className="divide-y divide-theme-divider">
              {filteredRecords.map((r) => (
                <tr key={r.id} className="hover:bg-theme-surface-2/50 transition-colors">
                  <td className="p-3">
                    <div className="font-bold text-theme-heading font-sans text-sm">{r.customerName}</div>
                    <div className="text-[10px] text-theme-muted flex items-center gap-1 mt-0.5">
                      <Store size={10} /> {r.storeLocation}
                    </div>
                  </td>
                  ...
                  <td className="p-3">
                    <div className="flex gap-1.5 text-[10px] font-bold">
                      <span className="px-1.5 py-0.5 bg-emerald-500/10 text-emerald-400 rounded">0-30: {r.agingBuckets.days0to30}</span>
                      <span className="px-1.5 py-0.5 bg-amber-500/10 text-amber-400 rounded">31-60: {r.agingBuckets.days31to60}</span>
                      <span className="px-1.5 py-0.5 bg-rose-500/10 text-rose-400 rounded">61+: {r.agingBuckets.days61plus}</span>
                    </div>
                  </td>
