...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="relative min-h-[100dvh] w-full flex flex-col justify-between overflow-x-hidden font-sans select-none bg-slate-950 text-slate-100">
      ...
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(ellipse_90%_60%_at_50%_-10%,rgba(37,99,235,0.25),transparent_75%)]" />
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_85%_35%,rgba(6,182,212,0.12),transparent_55%)]" />
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_15%_75%,rgba(37,99,235,0.15),transparent_55%)]" />

      {/* 3. Luxury Boutique Ambient Backdrop with High-Fidelity Bokeh Blur (Eliminates Third-Party Brand Leaks & Pixelation) */}
      <div
      ...
      <div className="fixed inset-0 pointer-events-none bg-gradient-to-t from-slate-950 via-slate-950/70 to-slate-950/85" />
      <div className="fixed inset-0 pointer-events-none bg-gradient-to-r from-slate-950/90 via-transparent to-slate-950/90" />

      {/* 4. Global Top-Right Utility: Language Selector */}
      <div className="absolute top-3 right-4 sm:top-5 sm:right-8 z-30">
        <div className="relative">
          <button
            type="button"
            onClick={() => setIsLangOpen(!isLangOpen)}
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl border border-slate-700/80 bg-slate-900/85 backdrop-blur-md text-xs font-semibold text-slate-300 hover:border-blue-400 hover:text-white transition shadow-sm cursor-pointer"
          >
          ...
          <AnimatePresence>
            {isLangOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 5 }}
                className="absolute right-0 mt-1.5 w-36 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl py-1 z-40"
              >
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    ...
                    className="w-full text-left px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-800 transition cursor-pointer"
                  >
                    {lang.label}
                  </button>
                ))}
      ...
      {/* 5. Top Mobile Brand Banner (Visible only on < lg screens) */}
      <header className="relative z-20 w-full lg:hidden px-4 pt-3 pb-1 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <span className="font-display font-black text-2xl tracking-tight text-white">
            SM<span className="text-blue-500 font-extrabold">₹</span>ITI
          </span>
          <span className="text-[10px] font-mono font-bold text-slate-500 self-start mt-0.5">®</span>
          <span className="text-[10px] font-mono font-bold text-blue-400 bg-blue-950/60 border border-blue-800/50 px-2 py-0.5 rounded-full ml-1">
            Retail OS
          </span>
        </div>
      </header>
      ...
      {/* 6. Main Content Area */}
      <main className="relative z-20 flex-1 w-full max-w-[1520px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-3 sm:py-5 lg:py-6 flex flex-col justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 xl:gap-10 items-center">
          ...
          <motion.div
            ...
            className="order-2 lg:order-1 lg:col-span-6 xl:col-span-4 flex flex-col justify-center space-y-3.5 sm:space-y-4"
          >
            ...
            {/* 6 Core Feature Pillars (2-columns on mobile/tablet, 1-column on desktop) */}
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 sm:gap-2.5 max-w-xl lg:max-w-sm">
              {RETAIL_PILLARS.map((pillar, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group flex items-center space-x-2.5 sm:space-x-3 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl bg-slate-900/60 backdrop-blur-md border border-white/10 hover:border-blue-500/40 shadow-xs hover:shadow-md hover:bg-slate-900/85 hover:translate-x-1 transition-all duration-200"
                  >
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl bg-blue-950/70 border border-blue-800/40 flex-shrink-0 flex items-center justify-center text-blue-400 group-hover:scale-105 group-hover:bg-blue-600 group-hover:text-white transition-all">
                      <IconComp size={15} strokeWidth={2.2} />
                    </div>
                    <div className="min-w-0">
                      <h3 className="text-[10.5px] sm:text-xs font-bold text-white leading-tight">
                        {pillar.title}
                      </h3>
                      <p className="text-[9px] sm:text-[10px] text-slate-400 leading-tight mt-0.5 line-clamp-1">
                        {pillar.desc}
                      </p>
                    </div>
                  </motion.div>
          ...
          {/* COLUMN 2: Canonical Glassmorphic Authentication Card (Order 1 on mobile, Order 2 on Desktop) */}
          <motion.div
          ...
          >
            <div className="w-full max-w-[440px] bg-slate-900/95 backdrop-blur-2xl border border-white/15 rounded-2xl sm:rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.6)] overflow-hidden relative">
              
              {/* Canonical Decorative Top Accent Stripe */}
              <div className="h-1.5 bg-blue-600 w-full" />

              <div className="p-6 sm:p-7 md:p-8">
                {/* Header: Exact Canonical Parity (Uncompressed Full-Width Subtitle) */}
                <div className="flex items-center space-x-3.5 mb-6">
                  <div className="w-12 h-12 rounded-xl bg-blue-600 flex-shrink-0 flex items-center justify-center font-bold text-xl font-display text-white border border-blue-500 shadow-md">
                    S
                  </div>
                  <div className="min-w-0">
                ...
                {/* Canonical Authentication Form */}
                <form onSubmit={handleSubmit} className="space-y-4">
                  {/* Operator ID Field */}
                  <div>
                    <label
                      htmlFor="login-username"
                      className="block text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider mb-1.5"
                    >
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        disabled={loading}
                        className="w-full bg-slate-800/80 border border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 font-semibold transition-all"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        disabled={loading}
                        className="w-full bg-slate-800/80 border border-slate-700 rounded-xl pl-10 pr-10 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 font-semibold transition-all"
                        placeholder="••••••••"
                      />
                      <button
                        type="button"
                  ...
                  <button
                    ...
                    id="btn-login-submit"
                    disabled={loading}
                    className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold font-display rounded-xl shadow-lg border border-blue-500 hover:border-blue-400 transition-all flex items-center justify-center space-x-2 text-xs sm:text-sm cursor-pointer select-none disabled:opacity-50 mt-1"
                  >
                ...
                {/* Canonical Quick Select Demo Persona Section */}
                <div className="mt-5 pt-1">
                  <div className="relative flex items-center justify-center mb-3">
                    <div className="border-t border-slate-800 absolute inset-x-0" />
                    <span className="relative bg-slate-900 px-3 text-[10px] font-mono text-slate-400 uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      ...
                      className={`py-1.5 px-2 rounded-lg border text-[10px] sm:text-xs font-semibold text-center transition-all cursor-pointer ${
                        activePersona === "Admin"
                          ? "border-blue-500 bg-blue-950/60 text-blue-300 shadow-xs"
                          : "border-slate-700 bg-slate-800/80 text-slate-200 hover:border-blue-400 hover:bg-slate-700/80"
                      }`}
                    >
                      Admin
                    </button>
              ...
              {/* Canonical Card Footer Security & Version Bar */}
              <div className="bg-slate-900/90 px-6 py-3 border-t border-slate-800 flex items-center justify-between text-[10px] text-slate-400 font-mono">
                <div className="flex items-center space-x-1.5">
          ...
          >
            {/* Majestic Slogan */}
            <div className="space-y-1 font-display font-black text-2xl xl:text-3xl text-white uppercase tracking-[0.15em] leading-snug drop-shadow-sm select-none">
              <div>ONE</div>
      ...
      {/* 7. Fluid Sapphire Wave Ribbon (Anchored directly above footer, spans 100% width cleanly) */}
      ...
      {/* 8. Bottom Enterprise Capability Dock (Responsive Grid on Mobile, Flex Row on Desktop) */}
