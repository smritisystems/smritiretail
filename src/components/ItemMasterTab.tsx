                ...
                {showRangeFilter && (
                  <div className="bg-[#141720] border border-indigo-500/30 rounded-2xl p-4 space-y-3 animate-in fade-in duration-150">
                    <div className="flex items-center justify-between border-b border-indigo-500/20 pb-2">
                      ...
                      <button onClick={() => { setMinStockFilter(""); setMaxStockFilter(""); setMinPriceFilter(""); setMaxPriceFilter(""); }} className="text-[10px] font-mono text-theme-muted hover:text-rose-400 transition-colors">Clear Range Filters</button>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div><label className="text-[9px] font-mono text-theme-muted block mb-1">Min Stock ({stockUnitLabel})</label><input type="number" min="0" value={minStockFilter} onChange={(e) => setMinStockFilter(e.target.value)} placeholder="e.g. 0" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-1.5 text-xs text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
                      <div><label className="text-[9px] font-mono text-theme-muted block mb-1">Max Stock ({stockUnitLabel})</label><input type="number" min="0" value={maxStockFilter} onChange={(e) => setMaxStockFilter(e.target.value)} placeholder="e.g. 100" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-1.5 text-xs text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
                      <div><label className="text-[9px] font-mono text-theme-muted block mb-1">Min Price (₹)</label><input type="number" min="0" value={minPriceFilter} onChange={(e) => setMinPriceFilter(e.target.value)} placeholder="e.g. 100" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-1.5 text-xs text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
                      <div><label className="text-[9px] font-mono text-theme-muted block mb-1">Max Price (₹)</label><input type="number" min="0" value={maxPriceFilter} onChange={(e) => setMaxPriceFilter(e.target.value)} placeholder="e.g. 5000" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-1.5 text-xs text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
                    </div>
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-[#141720] border border-indigo-500/30 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 bg-[#1a1e2b] border-b border-indigo-500/20 flex items-center justify-between">
              ...
              <button onClick={() => setViewingProduct(null)} className="p-1 rounded text-theme-muted hover:text-theme-heading hover:bg-theme-surface-hover transition"><X size={16} /></button>
            </div>
            <div className="p-6 overflow-y-auto space-y-5 font-mono text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-theme-surface-1 p-4 rounded-xl border border-theme-divider">
                <div><span className="text-[9px] text-theme-muted uppercase block">Category</span><span className="text-theme-heading font-bold">{viewingProduct.category}</span></div>
                <div><span className="text-[9px] text-theme-muted uppercase block">Barcode</span><span className="text-indigo-300 font-bold">{viewingProduct.barcode}</span></div>
                <div><span className="text-[9px] text-theme-muted uppercase block">Stock On Hand</span><span className="text-emerald-400 font-bold">{viewingProduct.stock} {stockUnitLabel}</span></div>
                <div><span className="text-[9px] text-theme-muted uppercase block">GST Tax %</span><span className="text-amber-400 font-bold">{viewingProduct.gstPercentage || 18}%</span></div>
              </div>
              <div className="grid grid-cols-3 gap-3 bg-theme-surface-1 p-4 rounded-xl border border-theme-divider text-center">
                <div><span className="text-[9px] text-theme-muted uppercase block">Buy Cost Price</span><span className="text-theme-body text-sm font-bold">₹{(viewingProduct.costPrice || Math.round(viewingProduct.price * 0.6)).toLocaleString("en-IN")}</span></div>
                <div><span className="text-[9px] text-theme-muted uppercase block">Selling Rate</span><span className="text-emerald-400 text-sm font-bold">₹{viewingProduct.price.toLocaleString("en-IN")}</span></div>
                <div><span className="text-[9px] text-theme-muted uppercase block">Max Retail Price</span><span className="text-theme-heading text-sm font-bold">₹{(viewingProduct.mrp || viewingProduct.price).toLocaleString("en-IN")}</span></div>
              </div>
              {viewingProduct.attributes && Object.keys(viewingProduct.attributes).length > 0 && (
                <div className="bg-theme-surface-1 p-4 rounded-xl border border-theme-divider space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">{Object.entries(viewingProduct.attributes).map(([k, v]) => (<div key={k} className="flex justify-between border-b border-theme-divider pb-1"><span className="text-theme-muted">{k}:</span><span className="text-theme-heading font-bold">{v}</span></div>))}</div>
                </div>
              )}
            </div>
            <div className="px-6 py-3 bg-[#1a1e2b] border-t border-indigo-500/20 flex items-center justify-between">
              ...
              <button onClick={() => setViewingProduct(null)} className="px-4 py-1.5 bg-theme-surface-2 hover:bg-theme-surface-hover text-theme-body rounded-lg text-xs transition">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-[#141720] border border-indigo-500/40 w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-6 py-4 bg-[#1a1e2b] border-b border-indigo-500/20 flex items-center justify-between">
              ...
              <button onClick={() => setShowBulkEditModal(false)} className="p-1 text-theme-muted hover:text-theme-heading"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4 text-xs font-mono">
              <div><label className="text-[10px] text-theme-muted block mb-1">Set Category for All Selected</label><select value={bulkCategory} onChange={(e) => setBulkCategory(e.target.value)} className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-2 text-theme-heading font-mono outline-none focus:border-indigo-500"><option value="">-- Keep Existing Categories --</option>{categories.filter(c => c !== "All").map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              <div><label className="text-[10px] text-theme-muted block mb-1">Adjust Selling Price By % (+10% or -5%)</label><input type="number" value={bulkPriceChangePercent} onChange={(e) => setBulkPriceChangePercent(e.target.value)} placeholder="e.g. 10 for +10% increase, -5 for discount" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-2 text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
              <div><label className="text-[10px] text-theme-muted block mb-1">Set GST Tax Rate %</label><select value={bulkGst} onChange={(e) => setBulkGst(e.target.value)} className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-2 text-theme-heading font-mono outline-none focus:border-indigo-500"><option value="">-- Keep Existing GST % --</option><option value="0">0% GST</option><option value="5">5% GST</option><option value="18">18% GST</option><option value="40">40% GST</option></select></div>
              <div><label className="text-[10px] text-theme-muted block mb-1">Add Stock Units to On-Hand (+Qty)</label><input type="number" value={bulkStockAdd} onChange={(e) => setBulkStockAdd(e.target.value)} placeholder="e.g. 50 (adds 50 units to stock on hand)" className="w-full bg-theme-surface-2 border border-theme-divider rounded-lg px-3 py-2 text-theme-heading font-mono outline-none focus:border-indigo-500" /></div>
            </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-[#141720] border border-amber-500/40 w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-6 py-4 bg-[#1a1e2b] border-b border-amber-500/20 flex items-center justify-between">
              ...
              <button onClick={() => setShowSuggestBestModal(false)} className="p-1 text-theme-muted hover:text-theme-heading"><X size={16} /></button>
            </div>
            <div className="p-6 space-y-4 text-xs font-mono text-theme-body leading-relaxed">
              ...
              <ul className="space-y-2 text-[11px] list-disc pl-4 text-theme-muted">
                <li><strong className="text-theme-heading">Margin Compliance:</strong> Enforces minimum 25% gross margin over buy cost on all products.</li>
                <li><strong className="text-theme-heading">MRP Alignment:</strong> Ensures MRP is at least 20% above selling rate for price compliance.</li>
                <li><strong className="text-theme-heading">Stock Buffer Suggestion:</strong> Replenishes low stock (&lt; 5 {stockUnitLabel}) up to the recommended buffer of 25 {stockUnitLabel}.</li>
              </ul>
