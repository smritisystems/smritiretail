...
export const GrnReceiptTab: React.FC<GrnReceiptTabProps> = ({
  ...
  const [ewayBillDate, setEwayBillDate] = useState(() => new Date().toISOString().split("T")[0]);
    ...
    <div className="space-y-4" data-testid="workspace-costs-freight">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
          ...
          <div>
            <label className="block text-[11px] font-medium text-slate-600 dark:text-slate-400 mb-1">E-Way Bill No. (12-Digit)</label>
            <input
              type="text"
              maxLength={12}
              value={ewayBillNumber}
              onChange={(e) => setEwayBillNumber(e.target.value.replace(/\D/g, ""))}
              placeholder="e.g. 241098234512"
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1.5 font-mono font-bold text-slate-800 dark:text-slate-200"
            />
          </div>
          <div>
            <label className="block text-[11px] font-medium text-slate-600 dark:text-slate-400 mb-1">E-Way Bill Date</label>
            <input
              type="date"
              value={ewayBillDate}
    ...
    <div className="space-y-4" data-testid="workspace-review-post">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-2.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <span className="text-slate-400 text-[10px] block uppercase">E-Way Bill No.</span>
            <span className="font-mono font-bold text-slate-800 dark:text-slate-200">{ewayBillNumber || "--"}</span>
          </div>
        </div>
      </div>
      ...
      {subView === "create" && workspaceMode === "desktop" && (
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          <GrnDesktopTerminal
            ...
            onEwayBillNumberChange={setEwayBillNumber}
            ewayBillDate={ewayBillDate}
            onEwayBillDateChange={setEwayBillDate}
          />
        </div>
      )}
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="pt-3 flex justify-end gap-2">
                  <button
                    ...
                    className="px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                  >
                    ← Select Different GRN
                  </button>
                  <button
                    type="button"
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-slate-500 font-medium">
                    Select a verified Goods Receipt Note to generate its Vendor Purchase Bill:
                  </p>
                  <button
                    type="button"
                    onClick={loadReceipts}
                ...
                {savedReceipts.length === 0 ? (
                  <div className="py-8 text-center text-xs text-slate-400">
                    No Goods Receipt Notes available. Post a GRN first.
                  </div>
                ) : (
                  <div className="max-h-80 overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-lg divide-y divide-slate-100 dark:divide-slate-800">
                    {savedReceipts.map((r) => (
                      <div
                        key={r.id}
                        className="p-2.5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/60 transition text-xs"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedReceiptForBill(r);
                              setVendorBillNo(r.invoice_number || "");
                            }}
                            className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded font-bold text-[11px] shadow-2xs transition"
                          >
                            Bill GRN →
                          </button>
                        </div>
