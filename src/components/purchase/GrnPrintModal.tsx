      ...
      <div>
          ...
          <table className="w-full text-left border-collapse border border-slate-300 text-[10px]">
            <thead>
              <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-300">
                ...
                <th className="p-1.5 border-r border-slate-300 text-right font-extrabold text-emerald-800">Accepted</th>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-1.5 border-r border-slate-300 font-bold text-slate-900">{item.code || item.sku || "--"}</td>
                    <td className="p-1.5 border-r border-slate-300 font-sans font-medium text-slate-800">{item.name || "Item"}</td>
                    <td className="p-1.5 border-r border-slate-300 text-center text-slate-600">{item.size || "--"}/{item.color || "--"}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right text-slate-600">{Number(item.quantity_ordered || 0)}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right text-slate-900">{Number(item.quantity_received || 0)}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right text-rose-600">{Number(item.quantity_damaged || 0)}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right font-bold text-emerald-800">{accepted}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right">{billedRate.toFixed(2)}</td>
                    <td className="p-1.5 border-r border-slate-300 text-right text-indigo-700">{freightAlloc > 0 ? freightAlloc.toFixed(2) : "--"}</td>
                    <td className="p-1.5 text-right font-bold text-indigo-900 bg-indigo-50/30">{landed.toFixed(2)}</td>
                  </tr>
                );
              })}
            </tbody>
            <tfoot className="bg-slate-50 font-mono font-bold border-t border-slate-300 text-[10px]">
              <tr>
                <td colSpan={4} className="p-1.5 border-r border-slate-300 text-right font-sans">Total Quantities:</td>
                ...
                <td className="p-1.5 border-r border-slate-300 text-right text-emerald-800 font-extrabold">{totalAccepted}</td>
                <td colSpan={3} className="p-1.5 text-right font-sans text-slate-500 font-normal">Net Units Accepted</td>
              </tr>
            </tfoot>
          </table>
        </div>
              ...
              <tbody className="divide-y divide-slate-200 font-mono">
                {costComponents.map((c, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1.5 border-r border-slate-300 font-bold text-slate-900">{c.component_type}</td>
                    <td className="p-1.5 border-r border-slate-300 font-sans text-slate-700">{c.description || "--"}</td>
                    <td className="p-1.5 border-r border-slate-300 text-center text-slate-600">{c.allocation_method || "VALUE"}</td>
                    <td className="p-1.5 border-r border-slate-300 font-sans text-slate-600">{c.transporter_name || "--"} {c.document_no ? `(${c.document_no})` : ""}</td>
                    <td className="p-1.5 text-right font-bold text-slate-900">{Number(c.amount || 0).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-slate-50 font-mono font-bold border-t border-slate-300">
                <tr>
                  <td colSpan={5} className="p-1.5 border-r border-slate-300 text-right font-sans">Total Additional Landed Costs:</td>
                  <td className="p-1.5 text-right text-indigo-700">₹{totalAddons.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3 p-3 bg-indigo-50/50 border border-indigo-200 rounded-lg text-center font-mono mb-4">
          <div>
            ...
            <span className="font-bold text-xs text-slate-900">₹{totalMaterialsValue.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
          </div>
          <div>
            <span className="text-[9px] text-slate-500 uppercase font-sans block">Freight &amp; Addons</span>
            <span className="font-bold text-xs text-indigo-700">₹{totalAddons.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
          </div>
          <div>
            <span className="text-[9px] text-slate-500 uppercase font-sans block">Final Inventory Cost</span>
            <span className="font-extrabold text-xs text-emerald-800">₹{totalAcquisitionCost.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
          </div>
          <div>
            <span className="text-[9px] text-slate-500 uppercase font-sans block">Avg. Unit Landed Cost</span>
            <span className="font-extrabold text-xs text-slate-900">₹{avgLandedCost.toFixed(2)}</span>
          </div>
        </div>
  ...
  return (
    <>
      ...
      {typeof document !== "undefined" && createPortal(
        <div className="print-only-container bg-white text-slate-900" id="smriti-grn-print-root">
          {renderPrintableSlip()}
        </div>,
        document.body
      )}
