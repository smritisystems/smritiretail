  ...
  if (req.method === "POST" && url === "/api/auth/login") {
    try {
      ...
      if (user.status === "Locked") {
        jsonResponse(res, 403, { error: "Too many failed login attempts. Your account has been locked. Please contact your administrator." });
        return;
      }

      if (user.status !== "Active") {
        jsonResponse(res, 403, { error: `Access Denied: Your account status is '${user.status}'.` });
        return;
      }
      ...
      if (!verifyPassword(body.password, user.passwordHash)) {
        ...
        if (user.failedAttempts >= 5) {
          user.status = "Locked";
          user.lockedUntil = new Date(Date.now() + 30000).toISOString();
