...
- `src/components/MasterMgmtTab.tsx` imports and uses `MASTER_REGISTRY` and `GLOBAL_AUDIT_FIELDS`.
...
- `src/services/custPolicyEngine.ts` owns customer policy resolution via `resolveCustomerPolicy()` and `checkCreditStatus()`.
