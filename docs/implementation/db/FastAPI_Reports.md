...
- **Governance Alignment:** Update `.agents/AGENTS.md` to record the FastAPI backend system-of-record policy.
...
- `.agents/AGENTS.md` references deprecated PAL sections and MARIA/Redis mappings that no longer apply.
...
- Rewrite the Platform Abstraction Layer (PAL) section to match the current FastAPI/Postgres architecture.
