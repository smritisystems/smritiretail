...
## 1. Objective
Add automated column-to-field header mapping for the manual data entry grid (`ExcelGridEntrySec.tsx`) to allow non-technical operators to copy-paste Excel/CSV spreadsheets directly without requiring strict manual column alignment or matching.
...
## 3. Scope
Enhance the spreadsheet's `handlePaste` function in `ExcelGridEntrySec.tsx` to automatically parse column headers, clean up text tokens, identify matched labels, map dynamic core & custom attributes, and populate rows correctly.
...
Revert changes to `ExcelGridEntrySec.tsx` via Git.
...
- [Fdn_Excel_Grid.md](../../walkthrough/foundation/Fdn_Excel_Grid.md)
