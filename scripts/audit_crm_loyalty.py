...
DOC_OUTPUT = r"F:\SMRITRretailNX\docs\architecture\SMRITI_CRM_LOYALTY_SICE_ARCHITECTURE_v1.0.md"
DIST_DIR = r"F:\SMRITRretailNX\dist"

def audit_crm_loyalty_sice():
    print("============================================================")
    print("SMRITI CRM, LOYALTY & UNIVERSAL COMMISSION ENGINE (SICE) AUDIT")
    print("============================================================")
    ...
    # 2. Reconcile Tables
    crm_loyalty_sice_tables = [
        ("customers", "Customer Profile Master", "smriti001", "IMPLEMENTED"),
        ("customer_groups", "Customer Group & Credit Hold Policy", "smriti001", "IMPLEMENTED"),
        ("loyalty_tiers", "Loyalty Tiers (Bronze, Silver, Gold, Platinum)", "smriti001", "IMPLEMENTED"),
        ("loyalty_rules", "Loyalty Earning & Redemption Rules", "smriti001", "IMPLEMENTED"),
        ("loyalty_members", "Loyalty Program Members", "smriti001", "IMPLEMENTED"),
        ("loyalty_points_ledgers", "Transactional Points Ledger (Earn/Redeem/Reversal)", "smriti001", "IMPLEMENTED"),
        ("commission_programs", "Incentive & Commission Programs", "smritisys / smriti001", "IMPLEMENTED"),
        ("commission_rules", "Commission Calculation Rules (Sales/Referral/Driver)", "smritisys / smriti001", "IMPLEMENTED"),
        ("commission_participants", "Universal Person Role Assignments", "smriti001", "IMPLEMENTED"),
        ("commission_ledgers", "Transactional Commission Ledger (Earned/Reversed)", "smriti001", "IMPLEMENTED")
    ]

    df_arch = pd.DataFrame([
        {"Table Name": t[0], "Functional Domain": t[1], "Database Owner": t[2], "Status": t[3]} for t in crm_loyalty_sice_tables
    ])
    ...
    summary_metrics = pd.DataFrame([
        ...
        {"Metric": "Single Company DB Principle", "Value": "CRM + Loyalty + SICE co-located in smriti001 (0 Extra DBs)"},
        {"Metric": "Universal Person Principle", "Value": "Person holds multiple roles (Salesperson, Driver, Referrer, Agent)"},
        ...
        {"Metric": "Final Status", "Value": "CRM_LOYALTY_SICE_ARCHITECTURE_VERIFIED"}
    ])
    ...
    df_arch.to_excel(writer, sheet_name="CRM_LOYALTY_SICE_ARCH", index=False)
    summary_metrics.to_excel(writer, sheet_name="CRM_LOYALTY_SUMMARY", index=False)
    writer.close()
    ...
    for sheetname in ["CRM_LOYALTY_SICE_ARCH", "CRM_LOYALTY_SUMMARY"]:
        ws = wb[sheetname]
        ws.freeze_panes = "A2"
...
""")
    ...
    print("\nCRM / LOYALTY / SICE AUDIT RESULTS:")
