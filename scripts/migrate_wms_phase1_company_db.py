...
def migrate_company_database(db_name: str):
    print(f"\n============================================================")
    print(f"Migrating Company Database: {db_name} (Transactional)")
    print(f"============================================================")
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS product_batch_stocks (
                ...
                deleted_by VARCHAR(50),
                version INTEGER DEFAULT 1,
                CONSTRAINT chk_batch_stock_positive CHECK (quantity >= 0 AND reserved_quantity >= 0 AND damaged_quantity >= 0)
            );
        """)
        ...
        # 4. Create stock_transfers table with DB check constraints
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS stock_transfers (
                ...
                deleted_by VARCHAR(50),
                version INTEGER DEFAULT 1,
                CONSTRAINT chk_transfers_diff_warehouses CHECK (source_warehouse_id != dest_warehouse_id)
            );
        """)
        ...
        # 5. Create stock_transfer_items table with DB check constraints
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS stock_transfer_items (
                ...
                CONSTRAINT chk_transfer_item_quantities CHECK (
                    quantity_dispatched > 0 AND 
                    quantity_received >= 0 AND 
                    quantity_shortage >= 0 AND 
                    quantity_damaged >= 0
                )
            );
        """)
        ...
        # 8. Seed Opening Batch Stocks for all products with stock > 0
        cur.execute("""
            INSERT INTO product_batch_stocks (
                id, uuid, company_id, branch_id, product_id, warehouse_id, batch_no,
                mfg_date, expiry_date, mrp, purchase_rate, sale_rate, quantity,
                reserved_quantity, damaged_quantity, is_active, is_deleted, created_at, modified_at
            )
            SELECT 
                'pbs-init-' || p.id,
                gen_random_uuid(),
